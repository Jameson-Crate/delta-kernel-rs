...
- Catalog-managed snapshot benchmarks currently support only `"snapshotBuilder": "for"` with
  `"incrementalCrcReplay": "disabled"`.
...
to use. `SnapshotConstructionConfig` selects fresh or previous-snapshot construction and the
incremental CRC replay policy. Multiple configs can be applied to the same workload to compare
modes. Which configs run for a given benchmark is determined by the
