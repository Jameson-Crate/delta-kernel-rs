...
impl DeltaError {
    ...
    fn unclassified(source: Error) -> Self {
...
impl From<Error> for DeltaError {
    fn from(source: Error) -> Self {
        match source {
            ...
            source => Self::unclassified(source),
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn legacy_error_uses_safe_fallback_and_preserves_source() {
        let error = DeltaError::from(Error::generic("sensitive implementation detail"));
    ...
    #[test]
    fn structured_error_survives_legacy_internal_boundary() {
        let legacy_result: super::super::DeltaResult<()> =
            Err(DeltaError::invalid_cdc_range(4, 2).into());
        let error = super::super::v2::into_delta_result(legacy_result)
            .expect_err("invalid CDF range must fail");
