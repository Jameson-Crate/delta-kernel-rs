...
impl SnapshotBuilder {
    ...
    /// Creates a snapshot through the option 2 structured-error boundary.
    ///
    /// # Parameters
    ///
    ...
    /// # Errors
    ///
    /// Returns a specific [`DeltaError`] for migrated conditions. Other
    /// failures use `DELTA_KERNEL_UNCLASSIFIED` and retain the legacy error as their source.
    pub fn build_v2(self, engine: &dyn Engine) -> crate::error::v2::DeltaResult<SnapshotRef> {
        crate::error::v2::into_delta_result(self.build(engine))
    }
...
mod tests {
    ...
    mod catalog_managed_tests {
        ...
        async fn test_non_contiguous_log_tail_errors(
            ...
            let result = builder.build_v2(engine.as_ref());
