...
fn generate_delta_error_conditions() {
    let catalog_paths = [
        "error-catalog/delta-error-classes.json",
        "error-catalog/kernel-error-classes.json",
    ];
    ...
    assert!(
        classes.windows(2).all(|pair| pair[0].name != pair[1].name),
        "duplicate Delta error condition across catalogs"
    );
...
fn render_error_conditions(classes: &[ErrorClass]) -> String {
    ...
    format!(
        "// @generated from the files in error-catalog.\n\
         \n\
