...
pub(crate) fn external_engine_error(
    error: impl std::error::Error + Send + Sync + 'static,
) -> EngineError {
    EngineError::External(Box::new(error))
}

pub(crate) fn object_store_engine_error(error: delta_kernel::object_store::Error) -> EngineError {
    match error {
        delta_kernel::object_store::Error::NotFound { path, .. } => EngineError::FileNotFound(path),
        delta_kernel::object_store::Error::AlreadyExists { path, .. } => {
            EngineError::FileAlreadyExists(path)
        }
        error => external_engine_error(error),
    }
}

pub(crate) fn delta_to_engine_error(error: Error) -> EngineError {
    match error {
        Error::Engine(error) => error,
        Error::Backtraced { source, .. } => delta_to_engine_error(*source),
...
pub(crate) fn wrap_engine_error(error: Error) -> Error {
    Error::Engine(delta_to_engine_error(error))
}
...
impl<E: TaskExecutor> DefaultEngine<E> {
    ...
    pub async fn write_parquet(
        ...
        let logical_to_physical_expr = self
            .evaluation_handler()
            ...
            .map_err(Error::Engine)?;
        let physical_data = logical_to_physical_expr
            .evaluate(data)
            .map_err(Error::Engine)?;
        self.raw_parquet
